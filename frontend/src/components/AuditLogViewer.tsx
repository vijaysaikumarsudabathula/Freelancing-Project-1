  ...
  return (
    <div className="w-full h-full flex flex-col bg-white rounded-lg border border-gray-100 overflow-hidden">
      ...
      <div className="flex justify-between items-center p-6 border-b">
        <h2 className="text-2xl font-bold">Audit & Activity Log</h2>
        {onClose && <button onClick={onClose} className="px-4 py-2 rounded border hover:bg-gray-50">Close</button>}
      </div>

      {/* Filter */}
      <div className="px-6 py-4 bg-[#FAF9F6] border-b">
        <input
          type="text"
          placeholder="Filter by user ID or email..."
          value={filterUser}
          onChange={(e) => setFilterUser(e.target.value)}
          className="w-full p-3 rounded border border-gray-100 outline-none focus:border-[#2D5A27]"
        />
      ...
      <div className="flex gap-0 border-b bg-white">
        <button
          ...
          className={`flex-1 py-3 px-4 text-sm font-bold uppercase tracking-widest border-b-2 transition ${activeTab === 'logins' ? 'border-[#2D5A27] text-[#2D5A27]' : 'border-transparent text-gray-400'}`}
        >
          🔐 Logins ({filteredLogins.length})
        </button>
        <button
          onClick={() => setActiveTab('activities')}
          className={`flex-1 py-3 px-4 text-sm font-bold uppercase tracking-widest border-b-2 transition ${activeTab === 'activities' ? 'border-[#2D5A27] text-[#2D5A27]' : 'border-transparent text-gray-400'}`}
        >
          📋 Activities ({filteredActivities.length})
        </button>
        <button
          onClick={() => setActiveTab('transactions')}
          className={`flex-1 py-3 px-4 text-sm font-bold uppercase tracking-widest border-b-2 transition ${activeTab === 'transactions' ? 'border-[#2D5A27] text-[#2D5A27]' : 'border-transparent text-gray-400'}`}
        >
