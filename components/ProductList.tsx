  ...
  return (
    <section id="shop" ref={listTopRef} className="py-32 relative">
      ...
      <div className="max-w-7xl mx-auto px-3 md:px-4 lg:px-8">
        <div className="flex flex-col lg:flex-row justify-between items-start mb-12 md:mb-24 gap-6 md:gap-12">
          <div className="max-w-xl w-full">
            <span className="text-[8px] md:text-[9px] lg:text-[10px] font-bold uppercase tracking-[0.4em] text-[#A4C639] mb-3 md:mb-4 block">{t.bestFromNature}</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-[#4A3728] mb-4 md:mb-6 serif leading-tight">{t.title1} <br /><span className="italic font-normal">{t.title2}</span></h2>
            <p className="text-[11px] md:text-sm lg:text-base text-[#5D7C52] font-medium leading-relaxed">
              {t.desc}
            </p>
          </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="flex items-center gap-2 md:gap-4 min-w-0">
                  <span className="text-lg md:text-xl flex-shrink-0">{activeCategoryData.icon}</span>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
              {paginatedProducts.map(product => {
                ...
                return (
                  <div key={product.id} className="bg-white rounded-[3rem] p-3 md:p-4 lg:p-6 shadow-sm border border-[#2D5A27]/5 hover:shadow-2xl transition-all duration-500 flex flex-col group relative">
                    {/* Wishlist Heart Icon */}
                    <button 
            ...
            {/* Pagination Controls */}
            {totalPages > 1 && (
              <div className="mt-12 md:mt-24 flex flex-col sm:flex-row justify-center items-center gap-2 md:gap-4 overflow-x-auto pb-2">
                <button
                  disabled={currentPage === 1}
                  onClick={() => handlePageChange(currentPage - 1)}
                  className={`px-3 md:px-6 py-2 md:py-4 rounded-lg md:rounded-xl text-[8px] md:text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${currentPage === 1 ? 'opacity-20 cursor-not-allowed' : 'bg-white border border-[#2D5A27]/10 text-[#2D5A27] hover:bg-[#FAF9F6]'}`}
                >
                  {t.prev}
                </button>
                  ...
                  {Array.from({ length: totalPages }).map((_, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl text-[8px] md:text-[10px] font-black transition-all ${currentPage === page ? 'bg-[#2D5A27] text-white shadow-xl scale-110' : 'bg-white border border-[#2D5A27]/5 text-[#2D5A27]/40 hover:text-[#2D5A27] hover:bg-[#FAF9F6]'}`}
                      >
                        {page}
                      </button>
                    );
                ...
                <button
                  ...
                  onClick={() => handlePageChange(currentPage + 1)}
                  className={`px-3 md:px-6 py-2 md:py-4 rounded-lg md:rounded-xl text-[8px] md:text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${currentPage === totalPages ? 'opacity-20 cursor-not-allowed' : 'bg-white border border-[#2D5A27]/10 text-[#2D5A27] hover:bg-[#FAF9F6]'}`}
                >
                  {t.next}
                </button>
              </div>
